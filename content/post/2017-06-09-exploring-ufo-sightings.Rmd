...
```
*(The outliers from each year reliably include a handful of states from the west, like Washington and California. More on that later.)*
...
```{r sightings-glm2, include = FALSE, eval = FALSE}
census_regions <- read_csv("../../data/ufo-sightings/census-regions.csv")

demog <- demog %>% 
  left_join(census_regions, "state") %>% 
  mutate(census_region = factor(census_region, c("Northeast", "Midwest", "South", "West")))

sightings_pois_region <- glm(sightings ~ census_region, data = demog, family = poisson)

summary(margins::margins(sightings_pois_region))
```
...
What about other local characteristics? The author in the *Economist* piece seemed to enjoy the wry assertion that drunk (and probably rural) people are the ones stumbling around and reporting these things. I'm not inclined think alcohol consumption wouldn't be related to a person experiencing a "sighting", but I wanted to look closer at that assertion. At the same time, states and regions aren't (solely) reducible to their drinking habits, and I wanted to bring some other features to bear. In order to examine these relationships, I built a regression model with state-based counts of sightings from 2013. I decided to fit a Poisson regression, given that our dependent variable is a count (visually the distribution seems to match, but the variance is bigger than the mean, indicating some overdispersion). With our dependent variable set, I looked at the following group of predictors:
...
In each plot, every state is represented as a single dot, with the number of sightings increasing along the y-axis, and the variable of interest along the x-axis. I've added a trend line to each plot to help summarize the relationships. Flat lines indicate that the relationship is basically random. This seems to be largely the case for most of our characteristics, but, population size and cost of rent might be showing a hint of something. Of note, our binge-drinking prevalence variable doesn't appear to be related to the number of sightings at all. We can be a little more specific by letting the regression model simplify some of this for us.
...
Okay, last plot, I promise! The main benefit of a regression model is that it lets us understand how an individual variable in our data contributes to the outcome we're interested in. The plot above shows the average marginal effects (AME) from each variable in our model, allowing us to estimate how the count of sightings rises or falls as we increase the values of our predictors. What we're seeing is that income, rent, and population size appear to be fairly related with our outcome. If we imagine we could pluck out an "average" state, and magically increase its population size by a standard-deviation above the country's average (holding the other variables constant), we'd expect to see around 72 additional sightings. Alternatively, if we did the same for the median rent, we'd expect to see 33 fewer sightings. Considering the average number of sightings was around 120, that's a fairly substantial impact.

...
Okay, last plot, I promise! The main benefit of a regression model is that it lets us understand how an individual variable in our data contributes to the outcome we're interested in. The plot above shows the average marginal effects from each variable in our model, allowing us to estimate how the count of sightings rises or falls as we increase the values of our predictors. What we're seeing is that income, rent, and population size appear to be fairly related with our outcome. If we imagine we could pluck out an "average" state, and magically increase its population size by a standard-deviation above the country's average (holding the other variables constant), we'd expect to see around 72 additional sightings. Alternatively, if we did the same for the median rent, we'd expect to see 33 fewer sightings. Considering the average number of sightings was around 120, that's a fairly substantial impact.

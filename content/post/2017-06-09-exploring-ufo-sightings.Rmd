...
*R code used for each of the graphics is available [here.](https://github.com/mooreaw/blogdown_source/blob/master/content/post/2017-06-09-exploring-ufo-sightings.Rmd)*
...
2. sightings are most most likely to occur
  - when people aren't working 
  - when people are likely to be drinking

For a short blurb, these takeaways seem tidy, but for a piece titled "Everything you need to know about UFOs", I felt like there wasn't enough to chew on. How much is alcohol really implicated in sightings of this kind? What about other local factors? These felt like glaring questions, but my musing was shelved when I closed the magazine. Lucky for me, Kaggle posted an updated version of the dataset to [download](https://www.kaggle.com/NUFORC/ufo-sightings), and I couldn't wait to look at some of these questions closer.
...
Next, I wanted to know whether there's a particular "season" for spotting UFOs. To get there, I created a rate of sightings per million people in the US population. This is a point where we can get a grip on just how rare sightings are, per-person-- in 2013 we're only seeing 2.4~ sightings per million people. You can see things get busier as we move toward the present, but there are also defined portions of the year that people are most likely to report sightings-- namely during the summer and fall. This lines up with what the *Economist* reported (i.e. sightings happen most frequently during evenings and weekends), given that these parts of the year tend to be when people spend the most time outside.
...
What about other local characteristics? The author in the *Economist* piece seemed to enjoy the wry assertion that drunk (and probably rural) people are the ones stumbling around and reporting these things. I'm not inclined think alcohol consumption wouldn't be related to a person experiencing a "sighting", but I wanted to look closer at that assertion. At the same time, states and regions are obviously not reducible to their drinking habits, and I wanted to bring some other features to bear. In order to examine these relationships, I built a regression model with state-based counts of sightings from 2013. In order to measure the relationships accurately, I used a Poisson regression rather than a standard linear model, given what the data looked like (`r sparkline::sparkline(demog$sightings, type = "bar")` Mean: `r mean(demog$sightings, na.rm = TRUE) %>% round(2)`, SD: `r sd(demog$sightings, na.rm = TRUE) %>% round(2)`). With our dependent variable set, I looked at the following group of predictors:
...
In each plot, every state is represented as a single dot, with the number of sightings increasing along the y-axis, and the variable of interest along the x-axis. I've added a trend line to each plot to help emphasize the relationships. Flat lines indicates that the relationship is equivocal or basically random. This seems to be largely the case for most of our characteristics, but, population size and cost of rent might be showing a hint of something. Of note, our binge-drinking prevalence variable doesn't appear to be related to the number of sightings at all. We can be a little more specific by letting the regression model simplify some of this for us. 
...
Okay, last plot, I promise! The main benefit of a regression model is that it lets us understand how an individual variable in our data contributes to the outcome we're interested in. The plot above show the average marginal effects from each variable in our model, allowing us to estimate how the count of sightings rises or falls as we increase the values of our predictors. What we're seeing is that income, rent, and population size appear to be fairly related with our outcome. If we imagine we could pluck out an "average" state, and magically increase its population size by a standard-deviation above the country's average (holding the other variables constant), we'd expect to see around 72 additional sightings. Alternatively, if we did the same for the median rent, we'd expect to see 33 fewer sightings. Considering the average number of sightings was around 120, that's a fairly substantial impact.

All said and done, it's easier to say what's *not* related to the number of reported sightings than to explain *why* certain state characteristics appear correlated. At the very least, more reports are coming from areas where the population tends to be larger, a little older, wealthier, and whose levels of openness and emotional stability are marginally higher. The best picture these results seem to paint is the western US. While minding necessary caveats about our observation level, and the fact that we're only using a single year of data, we might have enough to update our opinions about any obvious relationship between alcohol consumption and UFO sightings. Binge drinking and alcohol consumption in general aren't the same thing, but I would assume are positively related at the state level. Perhaps an index/score looking at binge drinking, state revenue from alcohol sales, and other variables together might clarify this relationship, but I'd argue that a state's level of binge drinking at least gives us a window into the region's permissiveness and attitudes about alcohol use. It could be the case that the population of people logging onto NUFORC's website to report sightings tend to drink more, but absent information about the reporters themselves, there's not enough to go off. By far, the biggest questions I have at this point center around what we don't know about the reporters. The fact that population size is positively related to sightings might be enough to say that reporters are mixed well enough into the population in general, but that isn't much of a starting point. Guess I'll just have to content myself with hopefully dodging the ecological fallacy.
